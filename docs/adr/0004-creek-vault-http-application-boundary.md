...
## Note, 2026-08-30 — the pin moves to 0.10.0; the capability axis grows to seven

The scheduled drift gate went red against a true positive. Adepthood
vendored contract 0.8.0 at upstream `349a56d`; Creek had since
published 0.9.0 and 0.10.0. `compare_upstream` reads `main` rather
than the pinned sha on purpose — "does the pin still match what Creek
publishes today" is the only question worth a network call — so the
red run was the gate working.

**What moved.** The bundle in `backend/tests/fixtures/creek_v1/` is
re-cut byte-for-byte from upstream `f9354bc` and `vendor.json` is
regenerated by the drift script's `snapshot` subcommand.
`CONTRACT_VERSION`, this bullet and the contract doc's version bullet
move together, as `test_contract_version_docs.py` requires.

Measured against the two trees rather than taken from the ticket:
77 manifest entries and 79 vendored files, up from 54 and 56; 27
schemas, up from 18; a 7 × 7 example matrix of 49 cells, up from
5 × 7 and 35. Seven files changed, twenty-three are new, and **no
local file was orphaned** — nothing upstream deleted, so the
re-vendor is purely additive plus seven republished files. The
ontology version is unchanged at `aptitude-wavelength/2026-05-23`.

**The capability enum grew from five names to seven**, adding
`drive-connector` (0.9.0) and `pipeline` (0.10.0). Both are now
members of `CreekCapability` and entries in the client's wire-name
table, and that is *all* they are: adepthood has no route, no client
method and no caller for either. Naming them is not a decision to
adopt them. It is forced by `_coerce_capability`, which drops an
advertised name it cannot map — so omitting them would make a vault
that offers a Drive connector indistinguishable from one that does
not, with no log line or metric anywhere saying so.

`pipeline` is deliberately **not** mapped onto the existing
`CreekCapability.CLASSIFY` member, even though both words describe
classification and the reuse would have kept the table injective for
free. Creek's `pipeline` is a whole-vault classify-and-link pass that
names no fragment; `CLASSIFY` is adepthood's per-entry concept whose
request shape is still unratified and whose method still refuses.
Mapping one onto the other would make `supports(CLASSIFY)` answer
true for a call that always raises — a degrade turned into a lie.
A test pins that apart.

**Two version-shape hazards, both checked and both clear.** A 0.10.0
pin declares minor `0.10`, which sorts *below* `0.8` as text and above
it as a version; every comparison in `creek_vault_client` splits on
the dot and matches components, so nothing orders these as strings.
And `CreekCapability` now has nine members while Creek publishes
seven — the enum happened to hold seven while Creek published five, so
a stale count assertion would have looked plausible against the new
contract rather than failing loudly.

**Not adopted here.** `related_praxis` and `related_eddies` are now
published on `ReflectionResponse` (both optional; `required` is
unchanged) with `RelatedPraxis`/`RelatedEddy` sibling schemas, and the
`drive-connector` and `pipeline` routes have ratified request and
response shapes. Reading any of them is the work of the issues that
depend on this re-vendor, not of this note. Vendoring a shape and
speaking it remain separate changes.
