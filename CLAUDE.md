...
- **When** adding or editing a command inside `scripts/**/*.sh` or a `run:` block in `.github/workflows/*.yml` that a `check-all.sh`, pre-commit, or workflow gate presents as enforcing, **do** run it once by hand to confirm it both executes and exits non-zero on a real violation, never swallow its exit code with `|| true` or `|| echo <default>` (branch on the code, so a transport failure stays distinguishable from a substantive zero), and add a meta-test under `backend/tests/scripts/` asserting the gate fails on a deliberately violating fixture. <!-- playbook added=2026-08-10 evidence=#2024,#2055,#2015,#2006,#2385 -->
- **When** adding or editing a `.github/workflows/*.yml` that runs on `schedule`, or an early-exit/stand-down gate inside one, **do** make the outcome legible from the run list — say in the run's display title or step summary which outcome occurred (ran / stood down and why / failed) and open-or-update a single tracking issue on failure — then prove it with one `workflow_dispatch` run confirming that jobs were actually created and the gate was reached. <!-- playbook added=2026-08-24 evidence=#2345,#2346,#2416 -->
