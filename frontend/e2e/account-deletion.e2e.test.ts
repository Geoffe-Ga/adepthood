...
describe('account-deletion journey against a live server', () => {
  ...
  it('erases the account on the retyped address and hands back a receipt', async () => {
    ...
    // This lane has a deployment-wide vault, but globalSetup binds that corpus
    // to exactly one pre-provisioned owner. This journey creates a later account
    // with no connection of its own, so its receipt must not claim somebody
    // else's vault or instruct this user to purge it.
    expect(receipt.vault.purged).toBe(false);
    expect(receipt.vault.configured).toBe(false);
    expect(receipt.vault.guidance).toContain('No Creek Vault');
  });
