...
import { daysBeforeStage, setAnchorDaysAgo, showProgress } from './programAnchor';
 ...
 *
 * The arrange is therefore out of band, through the shared helpers in
 * `programAnchor.ts`: the rewind runs against the lane's own throwaway database
 * through a backend module, and only the assert goes through the production
 * client, which is the half this lane exists to exercise.
 */
...
describe('map journey against a live server', () => {
  ...
  it('starts the program with the calendar and the record agreeing', async () => {
    ...
    const arranged = setAnchorDaysAgo(email, daysBeforeStage(FIRST_STAGE));
  ...
  it('moving the calendar past the first window does not by itself move the record', () => {
    const rewound = setAnchorDaysAgo(email, daysBeforeStage(SECOND_STAGE));
    ...
    const persisted = showProgress(email);
  ...
  it('carries a returning traveller into the window they walked into, not each one they missed', async () => {
    ...
    setAnchorDaysAgo(email, daysBeforeStage(FOURTH_STAGE));
