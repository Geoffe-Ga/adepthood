...
describe('RandomIntervalBellView — bell scheduling', () => {
  ...
  it('generates a constant-gap schedule when min === max', () => {
    ...
    expect(h.onMetadataChange).toHaveBeenLastCalledWith({
      mode: 'random_interval_bell',
      bells_struck: 3,
      interval_seconds: [30, 30, 30],
    });
  });

  it('does not regenerate the schedule while a session is already running', () => {
    const rng = jest.fn<() => number>(() => 0.5);
    const h = harness({ random: rng });
    const { element, rerender } = renderView(h);
    rerender(element(fakeState({ status: 'running', elapsedMs: 0 })));
    const callsAfterGeneration = rng.mock.calls.length;
    expect(callsAfterGeneration).toBeGreaterThan(0);
    // Drive the clock forward several times; the schedule was generated once
    // and must not be regenerated by the ticking elapsedMs.
    rerender(element(fakeState({ status: 'running', elapsedMs: 20_000 })));
    rerender(element(fakeState({ status: 'running', elapsedMs: 40_000 })));
    rerender(element(fakeState({ status: 'running', elapsedMs: 60_000 })));
    expect(rng.mock.calls.length).toBe(callsAfterGeneration);
  });
