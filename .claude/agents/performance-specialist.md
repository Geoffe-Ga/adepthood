...
description: "Profiles and optimizes performance-sensitive code — N+1 queries, hot FastAPI endpoints, large React Native lists/renders, algorithmic complexity. Select when the chief-architect flags a performance risk, and as the performance-dimension reviewer. Measure first; never trade correctness for speed."
level: 2
...
When invoked by code-review-orchestrator: flag N+1 patterns, unindexed lookups,
...
**Issue**: the Today screen loads habits then fetches each habit's streak in a
loop (N+1). Fix: a single batched query in the router/domain layer; assert the
query count drops in a test; confirm the screen renders identically.
