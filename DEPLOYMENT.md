...
| `TRUSTED_PROXY_CIDRS` | *(Railway's ingress range)* | Recommended — without it every client shares one rate-limit bucket and https redirects break; the per-client 600/minute ceiling then becomes a site-wide 600/minute across the whole API |
...
| `EXPO_PUBLIC_HABIT_DEMO_MODE` | `false` | Optional. Set to exact `true` only for a deliberate disconnected demo; never enable for a normal deployment because its habits and history are fictional |
...
| `WEB_CONCURRENCY` | No | `2` | Number of Uvicorn worker processes. All rate-limit state lives in each worker's memory (there is no shared store), so the effective per-deployment budget of every limit -- ambient floor, burst floors, per-client ceiling, declared route limits -- is `WEB_CONCURRENCY` x the stated limit |
...
| `EXPO_PUBLIC_HABIT_DEMO_MODE` | No | Defaults to `false`. The exact value `true` enables fictional habit fixtures only after a failed request with no real cache. Keep disabled outside a purpose-built demo; a successful empty response is always rendered truthfully. |
...
| Rate limit errors (429) | Too many requests from one client key: a single path's floor (60/minute, or its burst floor) or the per-client overall ceiling across every path (600/minute) | Rate limits reset on container restart. In-memory by design — not persistent across deploys, and per worker process, so the effective budget is `WEB_CONCURRENCY` x each limit. If every user is refused at once, check `TRUSTED_PROXY_CIDRS`: unset, all clients share one key and one ceiling |
